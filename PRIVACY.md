...
anywhere. Apple's built-in voices are in the same menu, and you can pick any of them instead.

## Network

Corpospeak makes no network connections. It has no analytics, no advertising, no accounts, and no
third-party services. The Kokoro speech model and the open-source library that loads it are
built into the app, so nothing is fetched to make it work.
